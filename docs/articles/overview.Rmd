...
The goal of **bsplus** is to provide access to some useful Bootstrap components for **rmarkdown** html-documents and **shiny** apps. The function families are:
...
In this package, a compound component is proposed:

- accordion_sidebar, a framework much like `shiny::tabsetPanel()` to organize your **shiny** apps.
...
The first step is to create a `bs_collapse()` with with an `id`. By default, this element is initially hidden. 

```{r}
bs_collapse(id = "ex_collapse", content = tags$p("Hi there")) 
```

Next, you create a tag, like a button, a then *attach* the `id` of the collapse to it.
...
To see more of the collapse function-family, please see [its article](https://ijlyttle.github.io/bsplus/docs/articles/collapse.html).

## Accordion

An accordion is a set of collapsible panels
...
Attach the id of the modal to a button.
...
Attach the id of the modal to another button.
...
tags$button(type = "button", class = "btn btn-primary","John Lennon") %>%
  bs_embed_popover(
    ...
    content = paste(
      "Although he wrote \"I Am the Walrus\",",
...
bs_carousel(id = "the_beatles", use_indicators = TRUE) %>%
  bs_append(
    bs_carousel_image(src = image_uri("img/john.jpg")),
    bs_carousel_caption("John Lennon", "Rhythm guitar, vocals")
  ) %>%
  bs_append(
    bs_carousel_image(src = image_uri("img/paul.jpg")),
    bs_carousel_caption("Paul McCartney", "Bass guitar, vocals")
  ) %>%
  bs_append(
    bs_carousel_image(src = image_uri("img/george.jpg")),
    bs_carousel_caption("George Harrison", "Lead guitar, vocals")
  ) %>%
  bs_append(
    bs_carousel_image(src = image_uri("img/ringo.jpg")),
    bs_carousel_caption("Ringo Starr", "Drums, vocals")
  ) 
```
