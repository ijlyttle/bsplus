...
In the example above, `body` is text; usually, you will want the `body` to HTML. One way to compose the HTML for the body is to write it as a Markdown file, then use the helper function `render_html_fragment()` to render it into into HTML. 
